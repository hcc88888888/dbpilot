...
export function ConnectionTestPanel({ instance, job, pending, tracking, onTest }: { instance: ManagedDatabaseInstance; job?: Job; pending: boolean; tracking: boolean; onTest(): void }) {
  const available = instance.capabilities.has('plugin_available');
  const testRunning = instance.connectionTestStatus === 'pending';
  const disabled = pending || testRunning || !available;
  const label = pending ? '正在下发…' : testRunning ? '测试进行中…' : '测试连接';
  return (
    <article className="panel">
      <header className="panel-header"><div><h2>连接测试</h2><p>测试通过 Agent 执行，仅使用已配置的凭据引用。</p></div><PermissionGate permission="database-instances:test" fallback={<span className="muted">无连接测试权限</span>}><button type="button" disabled={disabled} onClick={onTest}>{label}</button></PermissionGate></header>
      {!available ? <p className="notice notice--warning">当前插件不可用于连接测试。</p> : null}
