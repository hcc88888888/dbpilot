...
export function CandidateListPage({ api: override }: { api?: DefaultApi }) {
  ...
  const ignore = useIgnoreCandidate(api, project, filters);
  const accept = useAcceptCandidate(api, project, filters);
  ...
  return (
    <section className="management-page">
      ...
      <AsyncBoundary loading={query.isPending} error={query.error} onRetry={() => void query.refetch()}>
        {(query.data?.items.length ?? 0) === 0 ? <p role="status" className="empty-state">没有匹配的发现候选。</p> : <DataTable<DiscoveryCandidate & { id: string }> caption="发现候选" rows={(query.data?.items ?? []).map((candidate) => ({ ...candidate, id: candidate.candidateId }))} columns={[
          ...
          { key: 'etag', header: '操作', render: (row) => <PermissionGate permission="discovery:manage"><div className="row-actions"><button type="button" onClick={() => setToAccept(row)} aria-label={`纳管 ${row.candidateId}`}>纳管</button><button type="button" onClick={() => setToIgnore(row)} aria-label={`忽略 ${row.candidateId}`}>忽略</button></div></PermissionGate> },
        ]} />}
      </AsyncBoundary>
      <CandidateDetailDrawer candidate={evidence} onClose={() => setEvidence(undefined)} />
      <AcceptCandidateWizard candidate={toAccept} pending={accept.isPending} result={accept.data} onClose={() => { setToAccept(undefined); accept.reset(); }} onSubmit={(request) => toAccept && accept.mutate({ candidate: toAccept, request })} />
      <ConfirmDialog open={Boolean(toIgnore)} title="忽略发现候选" confirmLabel="确认忽略" onCancel={() => setToIgnore(undefined)} onConfirm={() => { if (toIgnore) ignore.mutate(toIgnore); setToIgnore(undefined); }}><p>该候选将不再出现在默认待处理列表中。</p></ConfirmDialog>
