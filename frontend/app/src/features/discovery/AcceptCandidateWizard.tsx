...
import type { DiscoveryCandidate, ManagedDatabaseInstance } from '../../../../generated/api/dist/index.js';
...
export function AcceptCandidateWizard({
  ...
  onClose(): void;
  onSubmit(request: { displayName: string; databaseFamily: string; databaseVariant: string; normalizedEndpoint?: string; unixSocket?: string; credentialRef: string; tlsRef?: string }): void;
}) {
  ...
  const canContinue = step === 0 || step === 1 ? Boolean(endpoint || candidate.unixSocket) : step === 2 ? Boolean(variant.trim()) : step === 3 ? Boolean(credentialRef.trim()) : true;
  return (
    <Drawer open title="纳管数据库实例" onClose={onClose}>
      ...
      {step === 3 ? <div className="form-stack"><FormField label="凭据引用" name="accept-credential-ref" value={credentialRef} onChange={(event) => setCredentialRef(event.target.value)} hint="填写 Secret/Vault 引用，不填写密码。" /><FormField label="TLS 引用" name="accept-tls-ref" value={tlsRef} onChange={(event) => setTlsRef(event.target.value)} hint="可选，仅填写证书引用。" /></div> : null}
