...
AND pa.desired_state='running' AND di.management_status<>'retired'`
	...
	if err != nil || !validAuthorization(value, agent, request) || !authorizer.Fences.ExecutionLeaseActive(agent.AgentID, commandID, value.AuthorizedAt.UTC()) {
		return Authorization{}, ErrLeaseRejected
	}
