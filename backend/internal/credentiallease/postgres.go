...
JOIN plugin_observations po ON po.tenant_id=pa.tenant_id AND po.project_id=pa.project_id AND po.assignment_id=pa.assignment_id AND (po.active_configuration_revision=pa.configuration_revision AND po.observed_operation_revision=pa.operation_revision AND (po.process_state='running' AND po.health='healthy' OR po.process_state='running' AND po.process_id>0 AND po.health='degraded' AND po.last_error_code='waiting_credentials' OR po.process_state='stopped' AND po.process_id=0 AND po.health IN ('unknown','unhealthy') AND po.last_error_code='agent_restart_reconciled' OR po.process_state='restarting' AND po.process_id=0 AND po.health='unhealthy' AND po.last_error_code='plugin_restart_failed') OR po.process_state='running' AND po.process_id>0 AND po.health IN ('healthy','degraded') AND po.last_error_code='plugin_upgrade_rolled_back')
WHERE pa.agent_id=$1 AND pa.assignment_id=$2 AND di.instance_id=$3 AND pa.configuration_revision=$4
AND pa.desired_state='running' AND di.management_status<>'retired'
AND EXISTS (
