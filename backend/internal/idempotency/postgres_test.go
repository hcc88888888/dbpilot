...
func TestPostgresStoreAtomicallyClaimsNewOrExpiredKeys(t *testing.T) {
	...
	t.Run("new", func(t *testing.T) {
		...
		mock.ExpectExec(regexp.QuoteMeta(insertClaimSQL)).
			WithArgs(key.Scope.TenantID, key.Scope.ProjectID, key.Actor, key.OperationID, key.IdempotencyKey, fingerprint, expires, now).
			WillReturnResult(sqlmock.NewResult(0, 1))
		mock.ExpectCommit()

		claim, err := NewPostgresStore(database).Claim(context.Background(), key, fingerprint, now, expires)
	...
	t.Run("expired", func(t *testing.T) {
		...
		mock.ExpectExec(regexp.QuoteMeta(reclaimExpiredSQL)).
			WithArgs(fingerprint, expires, now, key.Scope.TenantID, key.Scope.ProjectID, key.Actor, key.OperationID, key.IdempotencyKey).
			WillReturnResult(sqlmock.NewResult(0, 1))
		mock.ExpectCommit()
...
func TestPostgresStoreReplaysCompletedAndClassifiesDuplicateClaims(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			mock.ExpectBegin()
			mock.ExpectExec(regexp.QuoteMeta(insertClaimSQL)).WillReturnResult(sqlmock.NewResult(0, 0))
			mock.ExpectExec(regexp.QuoteMeta(reclaimExpiredSQL)).WillReturnResult(sqlmock.NewResult(0, 0))
...
func TestPostgresStoreCompletesAndAbortsOnlyMatchingProcessingClaim(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta(completeClaimSQL)).
		WithArgs(response.Status, `{"ETag":["\"8\""]}`, response.Body, now, key.Scope.TenantID, key.Scope.ProjectID, key.Actor, key.OperationID, key.IdempotencyKey, fingerprint, now).
		WillReturnResult(sqlmock.NewResult(0, 1))

	completed, err := NewPostgresStore(database).Complete(context.Background(), key, fingerprint, response, now)
	require.NoError(t, err)
	require.Equal(t, response, completed)

	mock.ExpectExec(regexp.QuoteMeta(abortClaimSQL)).
		WithArgs(key.Scope.TenantID, key.Scope.ProjectID, key.Actor, key.OperationID, key.IdempotencyKey, fingerprint).
		WillReturnResult(sqlmock.NewResult(0, 1))
	require.NoError(t, NewPostgresStore(database).Abort(context.Background(), key, fingerprint))
	require.NoError(t, mock.ExpectationsWereMet())
}
