...
func TestPreparedExpiryCommandWriteFailureRollsBackJobAndTarget(t *testing.T) {
	...
	require.Len(t, storedJob.TargetResults, 1)
	require.Equal(t, TargetQueued, storedJob.TargetResults[0].Status)
	storedCommand, err := repository.LookupCommand(ctx, message.ID)
	require.NoError(t, err)
...
func resetJobIntegrationSchema(t *testing.T, ctx context.Context, database *sql.DB) {
	...
	_, err := database.ExecContext(ctx, "CREATE TABLE IF NOT EXISTS dbpilot_schema_migrations (name TEXT PRIMARY KEY, applied_at TIMESTAMPTZ NOT NULL DEFAULT NOW())")
	require.NoError(t, err)
	_, err = database.ExecContext(ctx, "DROP TABLE IF EXISTS job_cancellation_snapshots, command_outbox, job_targets, jobs CASCADE")
	require.NoError(t, err)
