...
func TestPostgresDecommissionUsesScopedVersionCAS(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)UPDATE managed_hosts SET status = 'decommissioned'.*version = version \+ 1.*WHERE tenant_id = \$1 AND project_id = \$2 AND host_id = \$3 AND version = \$4 AND status <> 'decommissioned'.*RETURNING`).
		WithArgs(host.Scope.TenantID, host.Scope.ProjectID, host.ID, expected, at).
		WillReturnRows(hostRows(host))

	got, err := NewPostgresRepository(database).Decommission(context.Background(), host.Scope, host.ID, expected, at)
...
func TestPostgresDecommissionDistinguishesNotFoundFromVersionConflict(t *testing.T) {
	...
	_, err = NewPostgresRepository(database).Decommission(context.Background(), scope, "host-1", 2, time.Now().UTC())
