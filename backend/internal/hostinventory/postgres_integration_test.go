...
func TestHostPostgresIntegrationKeepsScopeRevisionAndObservationHistory(t *testing.T) {
	...
	_, err = repository.Decommission(ctx, scope, first.HostID, host.Version+1, heartbeatAt.Add(time.Second))
	require.ErrorIs(t, err, ErrConflict)
	host, err = repository.Decommission(ctx, scope, first.HostID, host.Version, heartbeatAt.Add(time.Second))
	require.NoError(t, err)
