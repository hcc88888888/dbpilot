...
func RunMigrations(ctx context.Context, database *sql.DB) error {
	if ctx == nil || database == nil {
		return errors.New("host inventory database and context are required")
	}
	entries, err := fs.Glob(migrationFiles, "migrations/*.sql")
	if err != nil {
		return fmt.Errorf("list host inventory migrations: %w", err)
	}
...
func applyMigration(ctx context.Context, database *sql.DB, path string) error {
	...
	if err != nil {
		return fmt.Errorf("read host inventory migration %s: %w", path, err)
	}
	transaction, err := database.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("begin host inventory migration %s: %w", path, err)
	}
	...
	if err := transaction.QueryRowContext(ctx, "SELECT EXISTS (SELECT 1 FROM dbpilot_schema_migrations WHERE name = $1)", name).Scan(&applied); err != nil {
		rollback()
		return fmt.Errorf("check host inventory migration %s: %w", path, err)
	}
	if !applied {
		...
		if _, err := transaction.ExecContext(ctx, body); err != nil {
			rollback()
			return fmt.Errorf("apply host inventory migration %s: %w", path, err)
		}
		if _, err := transaction.ExecContext(ctx, "INSERT INTO dbpilot_schema_migrations (name) VALUES ($1)", name); err != nil {
			rollback()
			return fmt.Errorf("record host inventory migration %s: %w", path, err)
		}
	}
	if err := transaction.Commit(); err != nil {
		return fmt.Errorf("commit host inventory migration %s: %w", path, err)
	}
	return nil
}
