...
func TestDatabaseInstanceValidationPostgresReconcilesNoResultTerminalsAndStaleDelivery(t *testing.T) {
	...
	require.NoError(t, jobs.RepairValidationTerminal(ctx, repairs[0].Scope, repairs[0].JobID, repairs[0].CommandID, repairs[0].AgentID, job.TargetResult{TargetID: repairs[0].AgentID, Status: job.TargetFailed, ErrorSummary: string(repairs[0].Result.ErrorCode), ResultSummary: "database instance connection validation failed", FinishedAt: &repairAt}, job.CommandFailed, repairAt))
	repairedJob, err := jobs.Get(ctx, scope, staleFence.ID)
	require.NoError(t, err)
	require.Equal(t, job.StatusFailed, repairedJob.Status)
	repairedCommand, err := jobs.LookupCommand(ctx, staleFenceCommandID)
	require.NoError(t, err)
	require.Equal(t, job.CommandFailed, repairedCommand.CommandStatus)
	repairs, err = instances.ListValidationJobRepairs(ctx, now.Add(time.Second), 8)
	require.NoError(t, err)
