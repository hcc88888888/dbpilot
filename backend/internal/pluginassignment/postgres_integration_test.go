...
func TestDatabaseInstanceValidationPostgresReconcilesNoResultTerminalsAndStaleDelivery(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, jobs.MarkCommandTerminal(ctx, scope, staleFenceCommandID, job.CommandTimedOut, now.Add(time.Second)))
	...
	repairs, err := instances.ListValidationJobRepairs(ctx, now.Add(time.Second), 8)
	require.NoError(t, err)
	require.Len(t, repairs, 1, "projection-terminal/Job-nonterminal half-commit must be discoverable")
	require.Equal(t, staleFence.ID, repairs[0].JobID)
	require.Equal(t, job.CommandTimedOut, repairs[0].Cause)
	repairAt := repairs[0].At
	halfCommittedJob, err := jobs.Get(ctx, scope, staleFence.ID)
	require.NoError(t, err)
	halfCommittedJob, err = jobs.Transition(ctx, job.Transition{Scope: scope, JobID: halfCommittedJob.ID, CurrentVersion: halfCommittedJob.Version, To: job.StatusDispatched, At: repairAt})
	require.NoError(t, err)
	halfCommittedJob, err = jobs.Transition(ctx, job.Transition{Scope: scope, JobID: halfCommittedJob.ID, CurrentVersion: halfCommittedJob.Version, To: job.StatusRunning, TargetResults: []job.TargetResult{{TargetID: repairs[0].AgentID, Status: job.TargetTimedOut, ResultSummary: "database instance connection validation timed out", FinishedAt: &repairAt}}, At: repairAt})
	require.NoError(t, err)
	require.NoError(t, jobs.RepairValidationTerminal(ctx, repairs[0].Scope, repairs[0].JobID, repairs[0].CommandID, repairs[0].AgentID, job.TargetResult{TargetID: repairs[0].AgentID, Status: job.TargetTimedOut, ResultSummary: "database instance connection validation timed out", FinishedAt: &repairAt}, job.CommandTimedOut, repairAt))
	repairedJob, err := jobs.Get(ctx, scope, staleFence.ID)
	require.NoError(t, err)
	require.Equal(t, job.StatusTimedOut, repairedJob.Status)
	repairedCommand, err := jobs.LookupCommand(ctx, staleFenceCommandID)
	require.NoError(t, err)
	require.Equal(t, job.CommandTimedOut, repairedCommand.CommandStatus)
	repairs, err = instances.ListValidationJobRepairs(ctx, now.Add(time.Second), 8)
	require.NoError(t, err)
