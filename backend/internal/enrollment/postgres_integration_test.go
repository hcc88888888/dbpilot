...
func TestEnrollmentPostgresGenerationZeroImportIsScopedAtomicAuditedAndPersistent(t *testing.T) {
	...
	corruptFingerprint := sha256.Sum256([]byte("corrupt-imported-host-leaf-" + suffix))
	_, err = database.ExecContext(ctx, `UPDATE managed_hosts SET active_certificate_fingerprint=$1,active_certificate_serial='7f',version=version+1,updated_at=$2 WHERE tenant_id=$3 AND project_id=$4 AND host_id=$5 AND agent_id=$6`, corruptFingerprint[:], now.Add(time.Second), scope.TenantID, scope.ProjectID, hostID, agentID)
	require.NoError(t, err)
	require.Error(t, configurer.ValidateGenerationZeroImports(ctx), "preflight must compare the exact imported generation, fingerprint, and serial with the Host active leaf")
	_, err = database.ExecContext(ctx, `UPDATE managed_hosts SET active_certificate_fingerprint=$1,active_certificate_serial=$2,version=version+1,updated_at=$3 WHERE tenant_id=$4 AND project_id=$5 AND host_id=$6 AND agent_id=$7`, fingerprint[:], serial, now.Add(2*time.Second), scope.TenantID, scope.ProjectID, hostID, agentID)
	require.NoError(t, err)
	_, err = database.ExecContext(ctx, `UPDATE agent_credential_imports SET certificate_serial='7f' WHERE tenant_id=$1 AND project_id=$2 AND host_id=$3 AND agent_id=$4`, scope.TenantID, scope.ProjectID, hostID, agentID)
	require.Error(t, err, "generation-zero import rows must be append-only")
	_, err = database.ExecContext(ctx, `DELETE FROM agent_credential_imports WHERE tenant_id=$1 AND project_id=$2 AND host_id=$3 AND agent_id=$4`, scope.TenantID, scope.ProjectID, hostID, agentID)
	require.Error(t, err, "generation-zero import rows must be immutable")
