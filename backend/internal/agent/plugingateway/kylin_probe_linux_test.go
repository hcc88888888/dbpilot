...
func TestKylinPrivatePluginGatewayProbe(t *testing.T) {
	...
	client, err := NewClient(ClientConfig{RuntimeRoot: runtimeRoot, Scope: MetricScope{TenantID: "tenant-1", ProjectID: "project-1", AgentID: "agent-1", HostID: "host-1"}, Store: store, Timeout: 3 * time.Second, Now: func() time.Time { return now }})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	configuration := PluginConfiguration{AssignmentID: "assignment-1", ConfigurationRevision: 4, Instances: []*pluginv1.PluginInstanceConfiguration{{InstanceId: "mysql-1", DatabaseVariant: "mysql", Endpoint: "127.0.0.1:3306"}, {InstanceId: "mysql-2", DatabaseVariant: "mysql", Endpoint: "127.0.0.1:3307"}}}
	require.NoError(t, session.ApplyConfiguration(context.Background(), configuration))
	require.NoError(t, session.CollectNow(context.Background(), []string{"mysql-1", "mysql-2"}, []string{"template-1"}))
	require.NoError(t, session.RunMetricStream(context.Background(), store))
	stats, err := store.Stats()
	require.NoError(t, err)
	require.Equal(t, 3, stats.PendingBatches)
...
func (fixture *gatewayFixture) CollectNow(_ context.Context, request *pluginv1.CollectPluginMetricsRequest) (*pluginv1.CollectPluginMetricsResponse, error) {
	return &pluginv1.CollectPluginMetricsResponse{Batches: []*pluginv1.PluginMetricBatch{fixture.batch("mysql-1", 1), fixture.batch("mysql-2", 2)}}, nil
}
func (fixture *gatewayFixture) StreamMetrics(_ *pluginv1.StreamPluginMetricsRequest, stream pluginv1.PluginRuntime_StreamMetricsServer) error {
	if err := stream.Send(fixture.batch("mysql-1", 3)); err != nil {
		return err
	}
	return nil
}
...
func (fixture *gatewayFixture) batch(instanceID string, sequence uint64) *pluginv1.PluginMetricBatch {
	return &pluginv1.PluginMetricBatch{PluginId: "mysql", PluginVersion: "1.0.0", DatabaseFamily: "mysql", DatabaseVariant: "mysql", InstanceId: instanceID, ConfigurationRevision: 4, TemplateId: "template-1", TemplateRevision: 1, CollectedAt: timestamppb.New(fixture.now), Sequence: sequence, CollectionStatus: pluginv1.PluginCollectionStatus_PLUGIN_COLLECTION_STATUS_SUCCEEDED, Samples: []*pluginv1.PluginMetricSample{{MetricName: "mysql.connections.current", Value: float64(sequence), Unit: "1", MetricType: pluginv1.PluginMetricType_PLUGIN_METRIC_TYPE_GAUGE, SampledAt: timestamppb.New(fixture.now)}}}
}
