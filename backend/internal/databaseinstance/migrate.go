...
func RunMigrations(ctx context.Context, database *sql.DB) error {
	if ctx == nil || database == nil {
		return errors.New("database instance database and context are required")
	}
	entries, err := fs.Glob(migrationFiles, "migrations/*.sql")
	if err != nil {
		return fmt.Errorf("list database instance migrations: %w", err)
	}
	...
	for _, migrationPath := range entries {
		...
		name := "databaseinstance/" + migrationPath
		var applied bool
		...
		if !applied {
			...
			if _, err := transaction.ExecContext(ctx, body); err != nil {
				rollback()
				return fmt.Errorf("apply database instance migration %s: %w", migrationPath, err)
			}
