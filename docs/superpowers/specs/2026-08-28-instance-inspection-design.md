...
本次主机 MVP 覆盖全部 Agent 目标，并且范围严格限定为 13 个版本 1 巡检项：CPU 使用率、每逻辑 CPU 负载、内存使用率、Swap 使用率、文件系统最高空间使用率、文件系统最高 inode 使用率、Agent 心跳新鲜度、指标新鲜度、spool 使用率、OOM 证据、时间同步、受信数据库进程存在性、错误日志摘要。文件系统两项使用全部成功读取挂载点的最坏值；任一已发现挂载读取失败时不发布完整聚合值。

只读文件系统/增长预测、网络连通性/连接错误、关键端口以及 Agent 策略状态明确进入下一主机功能阶段，不属于本 MVP，也不得在本阶段 UI、报告或完成声明中暗示已经覆盖。
